...
export function UseCreditsModal({
  ...
  onPayWithCard,
  isLoading = false,
}: UseCreditsModalProps) {
  const [selectedOption, setSelectedOption] = useState<"credits" | "card" | null>(null);

  const canCoverFully = credits >= paymentAmount;
  const remainingAfterCredits = Math.max(0, paymentAmount - credits);
  const creditsToUse = Math.min(credits, paymentAmount);
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-3">
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                  selectedOption === "card" ? "bg-primary/10" : "bg-muted"
                }`}>
                  <CreditCard className={`h-5 w-5 ${
                    selectedOption === "card" ? "text-primary" : "text-muted-foreground"
                  }`} />
                </div>
                <div className="flex-1">
                  <p className="font-medium">Pay with Card</p>
                  <p className="text-sm text-muted-foreground">
                    Pay the full ${paymentAmount.toFixed(2)} with your card
                  </p>
                </div>
                {selectedOption === "card" && (
                  <div className="w-5 h-5 rounded-full bg-primary flex items-center justify-center">
