          ...
          <div className="mx-auto grid max-w-7xl items-center gap-8 lg:grid-cols-2 lg:gap-10 xl:gap-12">
            <div>
              <h1 className="mb-6 text-5xl font-extrabold leading-tight md:text-6xl">
                {t("hero.title1")} <br />
                <span className="text-[#0e8ddd]">{t("hero.title2")}</span>
              </h1>
              ...
              <div className="relative z-40 flex max-w-2xl flex-col gap-3 sm:flex-row">
                ...
                <Link to="/courts" className="sm:flex-1">
                  <Button variant="outline" className="h-14 w-full justify-center rounded-xl border-slate-300 bg-white px-8 text-xl font-medium text-slate-800 hover:bg-slate-50">
              ...
              <div className="mt-8 flex gap-8 lg:mt-10 xl:mt-12">
                <div>
                  ...
                  <div className="text-xs uppercase tracking-wider text-slate-500">{t("hero.activePlayers")}</div>
                </div>
                <div className="border-l border-slate-200 pl-8">
                  <div className="text-2xl font-bold">150+</div>
                  <div className="text-xs uppercase tracking-wider text-slate-500">{t("hero.verifiedVenues")}</div>
                </div>
              </div>
            </div>
                ...
                <div className="absolute bottom-8 left-6 z-20 w-56 rounded-xl border border-slate-100 bg-white p-4 shadow-xl">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-0.5 text-[11px] text-slate-500">{t("hero.courtConfirmedDetail")}</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <section className="bg-slate-50 px-6 py-24">
          <div className="mx-auto max-w-7xl">
            <div className="mb-16 flex flex-col justify-between gap-4 md:flex-row md:items-end">
              <div>
                <span className="mb-4 block text-xs font-bold uppercase tracking-widest text-slate-400">{t("howItWorks.label")}</span>
                <h2 className="text-4xl font-extrabold">{t("howItWorks.title")}</h2>
              </div>
              <p className="max-w-xs text-sm text-slate-500">{t("howItWorks.subtitle")}</p>
            </div>
        ...
        <section className="py-20">
          <div className="relative overflow-hidden bg-[#0e8ddd] px-6 py-12 text-center text-white shadow-2xl shadow-[#0e8ddd]/25 md:px-10 md:py-20">
            <div className="relative z-10">
              ...
              <p className="mx-auto mb-10 max-w-2xl text-lg text-blue-50">{t("cta.description")}</p>
              <div className="flex flex-col justify-center gap-4 sm:flex-row">
                <Link to="/auth?tab=signup">
                  <Button className="rounded-2xl bg-white px-8 py-4 text-lg font-bold text-[#0e8ddd] hover:bg-blue-50">{t("cta.createAccount")}</Button>
                </Link>
                <Link to="/contact#lets-talk-courts">
                  <Button variant="outline" className="rounded-2xl border-white/40 bg-[#0b76bc] px-8 py-4 text-lg font-bold text-white hover:bg-[#09639e]">{t("cta.contactUs")}</Button>
                </Link>
              </div>
            </div>
        ...
        <div className="mx-auto max-w-7xl px-6">
          <div className="mb-20 grid gap-12 md:grid-cols-4">
            <div className="md:col-span-2">
              ...
              <p className="-mt-3 mb-6 max-w-xs text-sm leading-relaxed text-slate-500">{t("landingFooter.tagline")}</p>
            ...
            <div>
              <h4 className="mb-6 text-sm font-bold uppercase tracking-widest text-slate-400">{t("landingFooter.product")}</h4>
              <ul className="space-y-4 text-sm font-medium text-slate-600">
                <li><Link to="/courts#browse-courts" className="transition hover:text-blue-600">{t("landingFooter.browseCourts")}</Link></li>
                <li><Link to="/auth?tab=signup&role=player" className="transition hover:text-blue-600">{t("landingFooter.forPlayers")}</Link></li>
                <li><Link to="/auth?tab=signup&role=court_manager" className="transition hover:text-blue-600">{t("landingFooter.forCourtManagers")}</Link></li>
                <li><Link to="/groups" className="transition hover:text-blue-600">{t("landingFooter.communityGroups")}</Link></li>
              </ul>
            </div>

            <div>
              <h4 className="mb-6 text-sm font-bold uppercase tracking-widest text-slate-400">{t("landingFooter.company")}</h4>
              <ul className="space-y-4 text-sm font-medium text-slate-600">
                <li><Link to="/about#lets-talk-courts" className="transition hover:text-blue-600">{t("landingFooter.aboutUs")}</Link></li>
                <li><Link to="/contact#lets-talk-courts" className="transition hover:text-blue-600">{t("landingFooter.contact")}</Link></li>
                <li><Link to="/about#privacy" className="transition hover:text-blue-600">{t("landingFooter.privacyPolicy")}</Link></li>
                <li><Link to="/about#terms" className="transition hover:text-blue-600">{t("landingFooter.termsOfService")}</Link></li>
              </ul>
            </div>
          </div>

          <div className="flex flex-col items-center justify-between gap-4 border-t border-slate-100 pt-8 md:flex-row">
            <p className="text-xs text-slate-400">{t("landingFooter.allRightsReserved")}</p>
            <div className="flex gap-6 text-sm text-slate-500">
              <Link to="/about" className="transition hover:text-blue-600">{t("landingFooter.about")}</Link>
              <Link to="/contact" className="transition hover:text-blue-600">{t("landingFooter.contact")}</Link>
              <Link to="/courts" className="transition hover:text-blue-600">{t("landingFooter.browse")}</Link>
            </div>
          </div>
        </div>
      </footer>
